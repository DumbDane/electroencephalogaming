...
def training(inlet_eeg: StreamInlet, inlet_psypy: StreamInlet):
    ...
    while True:
        # TODO: do we need the timestamps and are we pushing them
        ...
        # TODO: decide which one below is easier to work with, needs to be a dict or series of some sort
        (
            c3_alpha_new,
            c3_beta_new,
            cz_alpha_new,
        ...
        if is_outlier(new_features, smpl_mean, smpl_sd, new_y):
            ...
            continue
        # TODO increment num_samples_seen here and then append to list before calling update
        smpl_mean, smpl_sd = update_mean_sd(
            features, smpl_mean, smpl_sd, num_samples_seen, labels
        )

        if not initial_trained:
            # TODO once the classifier is trained we want to only keep the "best" data part, what do we do before?
            data_raw["eeg_data"].append(features.values())
            ...
            if all([seen90, seen180, seen270]) > 10:
                ...
                clf.fit(data_raw["eeg_data"], labels)
            ...
            # supposedly, we recompute weights idk what that means for LDA
